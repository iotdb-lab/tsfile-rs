...
use snafu::{Snafu};
use crate::file::{footer, metadata};
...
pub fn parser_metadata<R: SectionReader>(reader: &R) -> Result<TsFileMetadata> {
    ...
    ensure!(file_size >= (FOOTER_SIZE as u64), InvalidTsFile{detail:"Size is smaller than footer".to_string()});
    ...
    ensure!(end_buf[4..] == [b'T', b's', b'F', b'i', b'l', b'e'], InvalidTsFile{detail:"Corrupt footer".to_string()});

    let metadata_len = BigEndian::read_i32(&end_buf[0..4]);
    ensure!( metadata_len >= 0,InvalidTsFile {detail:" Metadata length is less than zero".to_string()});
    ...
    ensure!( result == metadata_len as usize, InvalidTsFile {detail:"TsFileMetadata error".to_string()});
